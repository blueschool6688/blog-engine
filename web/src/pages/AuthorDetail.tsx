...
export async function loader({ request, params }: { request: Request, params: any }) {
  ...
  if (!nickname) return { author: null, posts: [], totalPosts: 0 };
  ...
  try {
    const [authorRes, postsRes] = await Promise.all([
      publicService.getAuthorByNickname(nickname),
      publicService.getPosts({ nickname, offset, limit })
    ]);
    ...
    return { author: null, posts: [], totalPosts: 0 };
  }
}

export const AuthorDetail: React.FC = () => {
  const { t, language } = useLanguage();
  const { author, posts, totalPosts } = useLoaderData() as any;
  ...
  if (!author) {
    return (
      <div className="py-12 text-center text-gray-500 max-w-6xl mx-auto px-4">
        <p>{t('no_authors')}</p>
      </div>
    );
  }
